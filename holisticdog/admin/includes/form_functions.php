...
echo "document.{$form}.{$combo_name}[1].selected = true";
echo "document.{$form}.{$combo_name}[1].selected = true";
//		document.getElementById($combo_name).selectedIndex;
		//$form.$combo_name[$value].selected;
		//document.{$form}.{$combo_name}[i].selected = true;
	...
	function create_combo_from_sql($table, $field_name, $query, $intro_text, $label_text, $selected_id) {
		$retval = "";
		$result = mysql_query($query);
		$retval .= '<label for="' . $table .'_id" id="' . $table .'_id_label">'.$label_text.'</label>' . "\n";
		$retval .=  '  <select name="' . $field_name .'" id="'. $field_name .'">' . "\n";
		$retval .=  '    <option value="">' . $intro_text . '</option>' . "\n";
		while ($row = mysql_fetch_assoc($result)) {
			$id = $row[$table . '_id'];
			$name  = $row[$table . '_title'];

			if ($id == $selected_id)
				$retval .=  "    <option value=\"{$id}\" SELECTED>{$name} ({$id})</option>\n";
			else
	...
	function create_combo_by_tablename($sourcetable, $targettable, $intro_text, $label_text, $starting_value) {
		$retval = "";
